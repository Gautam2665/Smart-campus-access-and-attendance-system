...
The MiniFASNet model demonstrates strong performance against standard 2D presentation attacks. The adaptive threshold adjustment based on environmental lighting (`analyze_image_quality`) meaningfully improved robustness in sub-optimal conditions. The 50ms polling interval for cloud verification (`poll_cloud_for_result`) produced minimal user-perceived latency, with the typical "scan-to-door-open" duration measured at approximately 2.8 seconds end-to-end for the fingerprint path.

The Haversine speed check proved highly effective at detecting credential sharing in simulation, instantly flagging impossible distances covered in short timeframes without any additional model compute cost.
